  ...
  function createNewTodo(e) {
    if ((e.type == "keydown" && e.key == "Enter") || e.type == "blur") {
      if (e.target.value.length > 0) {
        setAddedTodos([
          ...addedTodos,
          { text: e.target.value, id: uuid(), completed: false },
        ]);
        e.target.value = "";
      }
    }
  ...
  return (
    <main>
      ...
      <div className="options">
        <span className="itens-left">
          {showingTodos.length == 1
            ? `${showingTodos.length} item left`
            : `${showingTodos.length} items left`}
        </span>
        <div className="buttons">
          <button
            onClick={() => setShowFilter(1)}
            className={`${showFilter === 1 && "active"} filter`}
          >
            All
          </button>
          <button
            onClick={() => setShowFilter(2)}
            className={`${showFilter === 2 && "active"} filter`}
          >
            Active
          </button>
          <button
            onClick={() => setShowFilter(3)}
            className={`${showFilter === 3 && "active"} filter`}
          >
            Completed
          </button>
        </div>
